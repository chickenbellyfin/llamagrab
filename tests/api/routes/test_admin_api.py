...
@pytest.fixture
def user_to_verify(inmemory_db: Database):
...
@pytest.fixture
def user_to_admin(inmemory_db: Database):
  with inmemory_db.session() as db:
    user = models.User(
      id=77,
...
@pytest.fixture
def user_to_unadmin(inmemory_db: Database):
  with inmemory_db.session() as db:
    user = models.User(
      id=88,
...
def test_verify_user(test_client: TestClient, login_user_admin, user_to_verify, inmemory_db: Database):
  response = test_client.post('/api/admin/verify_user/55')
  assert response.status_code == status.HTTP_200_OK
  with inmemory_db.session() as db:
    db_user = db.query(models.User).filter_by(id=55).first()
...
def test_verify_user_already_verified(test_client: TestClient, login_user_admin, user_to_verify, inmemory_db: Database):
  with inmemory_db.session() as db:
    user_to_verify.tier = 'admin'
    db.merge(user_to_verify)
    db.commit()
  response = test_client.post('/api/admin/verify_user/55')
...
def test_make_admin(test_client: TestClient, login_user_super, user_to_admin, inmemory_db: Database):
  response = test_client.post('/api/admin/make_admin/77')
  assert response.status_code == status.HTTP_200_OK
  with inmemory_db.session() as db:
    db_user = db.query(models.User).filter_by(id=77).first()
...
def test_make_admin_already_admin(test_client: TestClient, login_user_super, user_to_admin, inmemory_db: Database):
  with inmemory_db.session() as db:
    user_to_admin.tier = 'admin'
    db.merge(user_to_admin)
    db.commit()
  response = test_client.post('/api/admin/make_admin/77')
  assert response.status_code == status.HTTP_400_BAD_REQUEST

def test_remove_admin(test_client: TestClient, login_user_super, user_to_unadmin, inmemory_db: Database):
  response = test_client.delete('/api/admin/make_admin/88')
  assert response.status_code == status.HTTP_200_OK
  with inmemory_db.session() as db:
    db_user = db.query(models.User).filter_by(id=88).first()
    assert db_user.tier == 'verified'
    assert db_user.limits.server_limit == 5
    assert db_user.limits.active_limit == 2
...
def test_remove_admin_not_admin(test_client: TestClient, login_user_super, user_to_unadmin, inmemory_db: Database):
  with inmemory_db.session() as db:
    user_to_unadmin.tier = 'verified'
    db.merge(user_to_unadmin)
    db.commit()
  response = test_client.delete('/api/admin/make_admin/88')
  assert response.status_code == status.HTTP_400_BAD_REQUEST
